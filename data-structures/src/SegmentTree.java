...
public class SegmentTree {
  ...
  public SegmentTree(int[] array, BiFunction<Integer, Integer, Integer> operation, int defaultValue) {
  ...
  private int findOutputIn(int segmentStartIndex, int segmentEndIndex, int rangeStartIndex, int rangeEndIndex, int segmentIndex) {
    ...
    return operation.apply(
      ...
      findOutputIn(mid + 1, segmentEndIndex, rangeStartIndex, rangeEndIndex, rightChildSegmentIndex)
    );
  }

  private int buildSegmentTree(int segmentStartIndex, int segmentEndIndex, int segmentIndex, int[] array) {
    ...
    int leftChildSegmentIndex = 2 * segmentIndex + 1;
    int rightChildSegmentIndex = 2 * segmentIndex + 2;
    treeArray[segmentIndex] = this.operation.apply(buildSegmentTree(segmentStartIndex, mid, leftChildSegmentIndex, array),
  ...
  public static void main(String[] args) {
    ...
    SegmentTree tree = new SegmentTree(array, (a, b) -> a + b, 0);
    tree.build();
    System.out.println(tree);

    System.out.println("Output in range 1, 3 is " + tree.findOutputIn(1, 3));
    System.out.println("Output in range 2, 5 is " + tree.findOutputIn(2, 5));
    System.out.println("Output in range 1, 6 is " + tree.findOutputIn(1, 6));
    System.out.println("Output in range 5, 6 is " + tree.findOutputIn(5, 6));
    System.out.println("Output in range 1, 4 is " + tree.findOutputIn(1, 4));
  }
}
